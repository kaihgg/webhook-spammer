...
"""
+ Fore.YELLOW + "https://github.com/kaihgg")
print("")
...
data = {
    "what do u want the webhool to spam": text # data for webhook as json
}
def send(i):
    res = requests.post(webhook, data=data) # sends data to webhook
    try:
        print(Fore.RED + 'lol gg ratelimite ' + str(res.json()["retry_after"]) + 'ms.')
